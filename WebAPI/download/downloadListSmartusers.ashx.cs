...
{
   /// <summary>
   /// Summary description for downloadListSmartusers
   /// </summary>
   public class downloadListSmartusers : IHttpHandler, System.Web.SessionState.IReadOnlySessionState
   {
      ...
      public void ProcessRequest(HttpContext context)
      {
         ...
         try
         {
            ...
            Write_folder_Stream(ref zStream, ref zEntry, getSmartUsers(context), Response, "ListOfSmartUsers" + strFileName);
            ...
            Response.AddHeader("content-disposition", "attachment; filename=SmartDots_SmartUsersExpertise_A" + HttpContext.Current.Session.SessionID.ToString() + "_" + DateTime.Now.Millisecond.ToString() + ".zip");
            zStream.Finish();
            zStream.Close();
            zStream.Dispose();
      ...
      public DataSet getSmartUsers(System.Web.HttpContext context)
      {
         ...
         string strSQL = "SELECT * FROM [dbo].[vw_SmartUsersExperties] ";
         if (Request.QueryString["tblCountryID"] != null )
         {
            if (Request.QueryString["tblCountryID"] != "0")
            {
               strSQL = strSQL + " WHERE tblCodeID_Country = " + Request.QueryString["tblCountryID"];
            }
         }
