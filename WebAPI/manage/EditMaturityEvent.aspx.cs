...
{
   public partial class EditMaturityEvent : System.Web.UI.Page
   {
      ...
      protected void Page_Load(object sender, EventArgs e)
      {
         ...
         lblListScales.Text = RunDBOperations.executeScalarQuery("select [dbo].[GetCodes](" + ddlMaturityScales.SelectedValue.ToString() + ")");
      ...
      public bool eventIsClosed(string strEventID)
      {
         int intOpen = (int)RunDBOperations.intExecuteScalarQuery("SELECT cast(Closed as int) as closed FROM[dbo].[tblEvent] where Published = 0 and tblEventID = " + strEventID);
         if (intOpen > 0)
         {
            return true;
         }
            ...
            try
            {
               ...
               using (SqlConnection cn = new SqlConnection(ConnectionString))
               {
                  ...
                  using (SqlCommand cmd = new SqlCommand(strSQL, cn))
                  {
                     ...
                     while (rdr.Read())
                     {
                        if (rdr["Purpose"].ToString().Contains("Age"))
                        {
                           Response.Redirect("EditEvent.aspx?tblEventID=" + strEventID);
                        }
      ...
      protected void ch_Visible(object sender, System.EventArgs e)
      {
         string a = "";
                     ...
                     else
                     {
                        try
                        {
                           using (SqlConnection conn = new SqlConnection(ConfigurationManager.ConnectionStrings["SmartDotsCS"].ConnectionString))
                           {
                              ...
                              strSQL = strSQL + "SELECT  top 1  " + strEventID + " AS EventID, SmartUser, 0 AS[Role], 0 as [ExpertiseLevel], 0 AS[ProvidesDataForAssessment] FROM dbo.[tblDoYouHaveAccess] WHERE     (SmartUser IN(SELECT     SmartUser FROM          dbo.tblDoYouHaveAccess WHERE(Email = '" + strUserEmail + "'))) ";
                              strSQL = strSQL + ") as t";
      ...
      protected void gv_SamplesAndImages_RowDataBound(object sender, GridViewRowEventArgs e)
      {
         ...
         {
            ImageButton imgAnna = (ImageButton)e.Row.FindControl("ImgEdit");
            string strVisibleduringevent = DataBinder.Eval(e.Row.DataItem, "visibleduringevent").ToString();
            if (strVisibleduringevent.ToUpper().Contains("TRUE"))
            {
                  imgAnna.ImageUrl = "~/images/cheked.png";
            }
            HyperLink imgButtonConnectedWithSamples = (HyperLink)e.Row.FindControl("imgSamplesImages");
            if (int.Parse(DataBinder.Eval(e.Row.DataItem, "CountImages").ToString()) > 0)
            {
               imgButtonConnectedWithSamples.ImageUrl = "~/SampleImages/" + SmartUtilities.getYearEvent(DataBinder.Eval(e.Row.DataItem, "tblEventID").ToString()) + "/" + DataBinder.Eval(e.Row.DataItem, "tblEventID").ToString() + "/" + DataBinder.Eval(e.Row.DataItem, "FileName").ToString();
               ...
               imgButtonConnectedWithSamples.NavigateUrl = "~/SampleImages/" + SmartUtilities.getYearEvent(DataBinder.Eval(e.Row.DataItem, "tblEventID").ToString()) + "/" + DataBinder.Eval(e.Row.DataItem, "tblEventID").ToString() + "/" + DataBinder.Eval(e.Row.DataItem, "FileName").ToString();
      ...
      protected void AddSamples_UploadComplete(object sender, AjaxControlToolkit.AjaxFileUploadEventArgs e)
      {
         ...
         string strEventID = Request.QueryString["tblEventID"].ToString();
         string strExtention = EditMaturityEvent.Right(fileName, 3);
      ...
      protected void bttChangeScale_Click(object sender, EventArgs e)
      {
         ...
         if (!string.IsNullOrEmpty(strEventID))
         {
            string strVisibleOrNotVisible = ddlVisitbleInvisible.SelectedValue.ToString();
            string strUpdateSQL;
            if (string.IsNullOrEmpty(txtSamplesScaleContains.Text))
            {
               strUpdateSQL = string.Format("update [dbo].[tblSmartImage] set Visible = {0}  where tblEventID = {1}", strVisibleOrNotVisible, strEventID);
            }
            else
            {
               strUpdateSQL = string.Format("update [dbo].[tblSmartImage] set Scale = {0}   FROM  dbo.tblSmartImage INNER JOIN dbo.tblSamples ON dbo.tblSmartImage.tblSampleID = dbo.tblSamples.tblSampleID  where tblEventID = {1} and sampleID like '%{2}%'", strVisibleOrNotVisible, strEventID, txtSamplesScaleContains.Text);
            }
         ...
         try
         {
            ...
            if (strEmailUser != null)
            {
               if (!RunDBOperations.checkIfUserIsEventManager(strEmailUser, Request.QueryString["tblEventID"].ToString()))
               {
                  ...
                  SmartUtilities.sendEmail(strEmailUser, "SmartDots event delegate appointment to the event: " + SmartUtilities.getEventField(Request.QueryString["tblEventID"], "NameOfEvent"), strBodyMessage);
      ...
      protected void ddlMaturityScales_SelectedIndexChanged(object sender, EventArgs e)
      {
         string strEventID = Request.QueryString["tblEventID"];
         string strMaturityScales = ddlMaturityScales.SelectedValue.ToString();

         RunDBOperations.runDBSmartDotsSQL("update [tblEventData] set value = " + strMaturityScales + " where tblTypeID = 3 and tblEventID = " + strEventID);
      }
